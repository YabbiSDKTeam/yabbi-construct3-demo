...
interface IRuntimeObjects {
	['show_ad']: IObjectClass<Ishow_ad>;
	['scaffold']: IObjectClass<Iscaffold>;
	['rewarded']: IObjectClass<Irewarded>;
	['logger']: IObjectClass<Ilogger>;
	['load_ad']: IObjectClass<Iload_ad>;
	['interstitial']: IObjectClass<Iinterstitial>;
	['events_title']: IObjectClass<Ievents_title>;
	['destroy_ad']: IObjectClass<Idestroy_ad>;
	['banner']: IObjectClass<Ibanner>;
	['back']: IObjectClass<Iback>;
	['app_bar_title']: IObjectClass<Iapp_bar_title>;
	['app_bar']: IObjectClass<Iapp_bar>;
	['Yabbi']: IObjectClass<IYabbi>;
}
...
interface Ishow_adVars extends VariableType {
	
}
interface Ishow_adBehaviors extends Record<string, IBehaviorInstance> {
	
}
interface Ishow_ad extends IButtonInstance {
	instVars: Ishow_adVars;
	behaviors: Ishow_adBehaviors;
}

interface IscaffoldVars extends VariableType {
	
}
interface IscaffoldBehaviors extends Record<string, IBehaviorInstance> {
	
}
interface Iscaffold extends IWorldInstance {
	instVars: IscaffoldVars;
	behaviors: IscaffoldBehaviors;
}

interface IrewardedVars extends VariableType {
	
}
interface IrewardedBehaviors extends Record<string, IBehaviorInstance> {
	
}
interface Irewarded extends IButtonInstance {
	instVars: IrewardedVars;
	behaviors: IrewardedBehaviors;
}

interface IloggerVars extends VariableType {
	
}
interface IloggerBehaviors extends Record<string, IBehaviorInstance> {
	
}
interface Ilogger extends ITextInstance {
	instVars: IloggerVars;
	behaviors: IloggerBehaviors;
}

interface Iload_adVars extends VariableType {
	
}
interface Iload_adBehaviors extends Record<string, IBehaviorInstance> {
	
}
interface Iload_ad extends IButtonInstance {
	instVars: Iload_adVars;
	behaviors: Iload_adBehaviors;
}

interface IinterstitialVars extends VariableType {
	
}
interface IinterstitialBehaviors extends Record<string, IBehaviorInstance> {
	
}
interface Iinterstitial extends IButtonInstance {
	instVars: IinterstitialVars;
	behaviors: IinterstitialBehaviors;
}

interface Ievents_titleVars extends VariableType {
	
}
interface Ievents_titleBehaviors extends Record<string, IBehaviorInstance> {
	
}
interface Ievents_title extends ITextInstance {
	instVars: Ievents_titleVars;
	behaviors: Ievents_titleBehaviors;
}

interface Idestroy_adVars extends VariableType {
	
}
interface Idestroy_adBehaviors extends Record<string, IBehaviorInstance> {
	
}
interface Idestroy_ad extends IButtonInstance {
	instVars: Idestroy_adVars;
	behaviors: Idestroy_adBehaviors;
}

interface IbannerVars extends VariableType {
	
}
interface IbannerBehaviors extends Record<string, IBehaviorInstance> {
	
}
interface Ibanner extends IButtonInstance {
	instVars: IbannerVars;
	behaviors: IbannerBehaviors;
}
...
interface Iback extends IButtonInstance {
	instVars: IbackVars;
	behaviors: IbackBehaviors;
}

interface Iapp_bar_titleVars extends VariableType {
	
}
interface Iapp_bar_titleBehaviors extends Record<string, IBehaviorInstance> {
	
}
interface Iapp_bar_title extends ITextInstance {
	instVars: Iapp_bar_titleVars;
	behaviors: Iapp_bar_titleBehaviors;
}

interface Iapp_barVars extends VariableType {
	
}
interface Iapp_barBehaviors extends Record<string, IBehaviorInstance> {
	
}
interface Iapp_bar extends ISpriteInstance {
	instVars: Iapp_barVars;
	behaviors: Iapp_barBehaviors;
}

interface IYabbiVars extends VariableType {
	
}
interface IYabbiBehaviors extends Record<string, IBehaviorInstance> {
	
}
interface IYabbi extends IWorldInstance {
	instVars: IYabbiVars;
	behaviors: IYabbiBehaviors;
}
